...
export class RefreshTokenCleanupService {
  ...
  async cleanupExpiredTokens() {
    const expiredTokens = await this.refreshTokenRepository.createQueryBuilder('refresh_token')
      .where('refresh_token.expiresAt < :now', { now: new Date() })
      .andWhere('refresh_token.isRevoked = false')
      .getMany();

    if (expiredTokens.length > 0) {
      console.log(`Cleaning up ${expiredTokens.length} expired refresh tokens.`);
